	...
	, m_addFileMenu(true)
{
	...
	m_mgr->GetTheApp()->Connect(XRCID("cd_open"), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(CodeDesigner::OnOpenWithCD), NULL, this);
	m_mgr->GetTheApp()->Connect(XRCID("cd_open"), wxEVT_UPDATE_UI, wxUpdateUIEventHandler(CodeDesigner::OnUpdateOpenWithCD), NULL, this);
	m_mgr->GetTheApp()->Connect(XRCID("cd_new_simplestatechart"), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(CodeDesigner::OnNewSCH), NULL, this);
	m_mgr->GetTheApp()->Connect(XRCID("cd_new_hierarchicalstatechart"), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(CodeDesigner::OnNewHSCH), NULL, this);
	m_mgr->GetTheApp()->Connect(XRCID("cd_new_classdiagram"), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(CodeDesigner::OnNewCD), NULL, this);

	m_mgr->GetTheApp()->Connect(wxEVT_TREE_ITEM_FILE_ACTIVATED, wxCommandEventHandler(CodeDesigner::OnOpenFile), NULL, this);

	m_mgr->GetTheApp()->Connect(wxID_ANY, wxEVT_PROC_TERMINATED, wxCommandEventHandler(CodeDesigner::OnCDTerminated), NULL, this);
...
void CodeDesigner::CreatePluginMenu(wxMenu *pluginsMenu)
{
	...
	m_mgr->GetTheApp()->Connect( XRCID("cd_settings"), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler( CodeDesigner::OnSettings ), NULL, this );
}
...
void CodeDesigner::HookPopupMenu(wxMenu *menu, MenuType type)
{
	...
	else if (type == MenuTypeFileView_File)
	{
		...
		if(m_addFileMenu)
		{
			wxMenuItem *item = new wxMenuItem(menu, XRCID("cd_open"), _("Open with CodeDesigner..."), wxEmptyString, wxITEM_NORMAL);
...
wxMenu* CodeDesigner::CreatePopupMenu()
{
	...
	item = new wxMenuItem(menu, XRCID("cd_new_simplestatechart"), _("New simple state chart..."), wxEmptyString, wxITEM_NORMAL);
	item->SetBitmap( wxBitmap(SimpleStateChartDiag_xpm) );
	menu->Append(item);

	item = new wxMenuItem(menu, XRCID("cd_new_hierarchicalstatechart"), _("New hierarchical state chart..."), wxEmptyString, wxITEM_NORMAL);
...
void CodeDesigner::OnOpenWithCD(wxCommandEvent& e)
{
	...
	if ( item.m_item.IsOk() && item.m_itemType == ProjectItem::TypeFile )
	{
		...
		{
			wxMessageBox(_("Please select a 'cdp' (CodeDesigner Project) file only"), _("CodeLite"), wxOK|wxCENTER|wxICON_INFORMATION);
			return;
		}
	}
}
...
void CodeDesigner::DoCreateCDProject(CDItemInfo& info)
{
	...
	//wxString templateFile(wxT("/home/michal/Src/Projekty/CodeLite_dev/CLInstall/share/codelite/templates/codedesigner/"));
	...
	{
		wxMessageBox(wxString::Format(_("Can't find CodeDesigner template file '%s'"), tmplFile.GetFullPath().c_str()), _("CodeLite"), wxOK|wxCENTER|wxICON_WARNING);
		return;
	}
	...
	if( proj )
	{
		...
		{
			wxMessageBox(wxString::Format(_("Failed to copy template file to '%s'"), cdpFile.GetFullPath().c_str()), _("CodeLite"), wxOK|wxCENTER|wxICON_WARNING);
			return;
		}
		...
		{
			wxMessageBox(wxString::Format(_("Failed to read file '%s'"), cdpFile.GetFullPath().c_str()), _("CodeLite"), wxOK|wxCENTER|wxICON_WARNING);
			return;
		}
		...
		{
			wxMessageBox(wxString::Format(_("Failed to write file '%s'"), cdpFile.GetFullPath().c_str()), _("CodeLite"), wxOK|wxCENTER|wxICON_WARNING);
			return;
		}
...
CDConnection::CDConnection(CDServer* server)
{
	m_Server = server;
}
...
CDServer::CDServer()
{
	m_Connections.DeleteContents( true );
}
